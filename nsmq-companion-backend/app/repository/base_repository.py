...
from app.model.equipment_type import EquipmentType
...
class BaseRepository:
    ...
    def read_all_by_uuid(self, uuid: str, eager=False):
        with self.session_factory() as session:
            query = session.query(self.model)
            if eager:
                for eager in getattr(self.model, "eagers", []):
                    query = query.options(joinedload(getattr(self.model, eager)))
            query = query.filter(self.model.asset_type_uuid == uuid).all()
            return query

    def get_equipment_by_uuid(self, equipment_uuid: str, eager=False):
        with self.session_factory() as session:
            query = session.query(EquipmentType)
            if eager:
                for eager in getattr(EquipmentType, "eagers", []):
                    query = query.options(joinedload(getattr(EquipmentType, eager)))
            query = query.filter(EquipmentType.equipment_uuid == equipment_uuid).first()
            if not query:
                return None
            return query
    ...
    def read_by_uuid(self, uuid: str, eager=False):
        with self.session_factory() as session:
            query = session.query(self.model)
            if eager:
                for eager in getattr(self.model, "eagers", []):
                    query = query.options(joinedload(getattr(self.model, eager)))
            query = query.filter(self.model.uuid == uuid).first()
            if not query:
                return None
            return query
    ...
    def update_by_uuid(self, uuid: str, schema):
        with self.session_factory() as session:
            session.query(self.model).filter(self.model.uuid == uuid).update(
                schema.dict(exclude_none=True)
            )
            session.commit()
            return self.read_by_uuid(uuid)
    ...
    def update_attr_by_uuid(self, uuid: str, column: str, value):
        with self.session_factory() as session:
            session.query(self.model).filter(self.model.uuid == uuid).update(
                {column: value}
            )
    ...
    def delete_by_uuid(self, uuid: str):
        with self.session_factory() as session:
            query = session.query(self.model).filter(self.model.uuid == uuid).first()
            if not query:
                raise NotFoundError(detail=f"not found uuid : {uuid}")
            session.delete(query)
            session.commit()